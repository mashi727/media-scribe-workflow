...
処理の流れは **各chを連結 → loudnorm → 映像へ個別ドリフト補正・同期 → L↔R 相対補正 → ステレオ結合 → mux**。
L/R を独立に映像へ合わせることで左右の開始時刻の違いを吸収し、さらに各chの補正残差の
食い違いで残る **L↔R の相対ドリフトを、R を L 基準にもう一段補正して密着**させる
（`sync.channel_drift_correct`、既定 on。ドリフトが小さい／TC 収録なら no-op）。
...
**各chの対映像残差が小さくても、L↔R がずれることはある。** 実際 8/23 では R の個別補正が
8ppm 残差を残し、L↔R が末尾で ~130ms ずれた。これを防ぐため、結合前に **R を L 基準で
相対ドリフト補正**する（`sync.channel_drift_correct`、既定 on。TC 収録などドリフトが
小さければ no-op）。この補正で通常は `channel_tolerance` 内に収まる。
...
### Chaptr のスペクトログラムを見ながら人が作る（本来の作業）

章立ては **Chaptr 上で、スペクトログラムを見ながら一から打つ**。これが本来想定している作業で、
ここを機械で置き換えようとしない（下の2節はその試みの記録）。

### `audio-timeline-plot` は俯瞰用（作業面ではない）

```bash
python3 bin/advanced/audio-timeline-plot <take_L.wav> --srt <work.srt> -o overview.png
```

1行30分・時間軸を揃えた3段（スペクトログラム／1秒ごとの dBFS／発話帯）を**1枚に**描く。
3.7時間で 9.3秒。時間軸は `H:MM`、補助線は1分ごと。

Chaptr は再生位置まわりを見る道具なので、**全体の構造を一望する**用途はこちらが担う。
読み方は **「音量あり かつ 字幕なし」＝演奏／「音量なし かつ 字幕なし」＝休憩／
「字幕あり」＝指揮者のトーク**。使いどころは次の3つ。

- 章立てを始める前に、その日の大きな流れ（休憩の位置・通しの回数）を把握する
- 閾値の当たりを付ける（`audio-features --report` と併用）
- 後から「この区間は何だったか」を確認する

### 自動生成の下書きは使わない

`segments-detect --chapters` は章立ての下書きを吐けるが、**実運用で採らないと決めた。**
理由は一致率そのものではなく、**下書きを直すより一から作るほうが速かった**（実測）こと。

一致率 **52.4%** では下書きのどの境界も信用できないので、人は結局すべてを検証する。
つまり**下書きは検証の手間を減らさず、削除と修正の手間だけを足す**。加えて、
もっともらしく見える誤った境界を受け入れてしまう錨効果が乗る。
スペクトログラムから作る場合は**検証と作成が同じ一つの動作**で、別工程が存在しない。

> 一般に、下書きが役に立つのは `検証+修正 < ゼロから作成` のときだけ。
> 作成側が視覚的で速いなら、下書きに要求される精度の水準は非常に高くなる。

誤りの内訳は次の2型（改良の手がかりとして記録しておく）。

- **休憩中のざわめきを演奏と誤検出**（実測2区間・33.5分）。高音圧・広帯域・ASR無出力で、
  音響特徴だけでは演奏と区別できない。判別には倍音構造や音符の立ち上がり密度が要る
- **演奏に重ねた指示でトークに倒れる**（実測約60分）

`segments-detect` の JSON 自体は `segments-review`（確認用の抜き出し）で使うので残す。
捨てたのは**章立ての下書きとしての用途**だけ。

### 試して採らなかった: 長い無音を目印にする

「全区間を分類するのはやめ、**確実な目印だけ**をスペクトログラム上に打つ」案を測定した。
高精度・低再現に割り切れば、見逃しは目が拾うので害がない、という筋書きだった。

**前提が成り立たなかった。** 8/9 の確定チャプター30境界に対する実測:

| 錨 | 許容 | 閾値 | 最小長 | 候補 | 適合率 | 再現率 |
|---|---|---|---|---|---|---|
| 終端 | ±15s | −35 dBFS | 5s | 126 | 17% | 57% |
| 終端 | ±30s | −35 dBFS | 5s | 126 | **22%** | 67% |
| 終端 | ±15s | −35 dBFS | 10s | 31 | 13% | 13% |
| 終端 | ±15s | −40 dBFS | 5s | 67 | 16% | 33% |

錨（無音の終端／始端／中点）と許容幅を変えても**適合率は最良で 22%**。
3.7時間に126個の目印は 106秒に1個で、これは目印ではなく雑音。

理由は次の1行に尽きる。**各境界の直前60秒で −45 dBFS を下回り続けた最長時間は、
中央値 2秒・最長 12秒**（30境界中2件は 0秒）。合奏の曲間は指揮者が話しているので、
**持続的な無音がそもそも存在しない**。境界の「静けさ」は1〜3秒の瞬間的な落ち込みでしかなく、
それは曲中のフレーズ間や休符でも同じだけ起きる。
**無音は境界に存在するが、境界に特有ではない。**

> 測定に10分。作っていたら20分＋その後ずっと誤報を見せ続けていた。
> **目印を出す前に、その目印が境界に特有かを測る。**

なお `audio-features` は閾値の当たりを付けたいときに使う（4.4h を 4.6秒）。
...
| [2] 章立ての確定 | 人の作業。数時間（**このワークフロー最大のコスト**） |
| [3] 原本の凍結 | **43秒**（無劣化カット） |
