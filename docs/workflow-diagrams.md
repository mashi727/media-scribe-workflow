...
| 入力 | YouTube URL | URL | - | 素材 |
| 中間 | ダウンロード動画 | .mp4 | yt-dlp | YouTube動画のローカルコピー |
| 中間 | チャプター付きmp4 | .mp4 | video-chapters | 編集済み動画 |
| 中間 | chapters.txt | .txt | video-chapters | チャプター情報 |
| 中間 | SRT（YouTube） | .srt | yt-srt | YouTube自動字幕 |
| 中間 | SRT（Whisper） | .srt | whisper-remote | Whisper文字起こし |
| 中間 | SRT + 話者 | .srt | whisperX（外部） | 話者特定付き |
| 最終 | スクリプト PDF | .pdf | AI + LaTeX | 一次資料、詳細記録 |
| 最終 | サマリー PDF | .pdf | AI + LaTeX | 二次資料、学習教材 |
| 最終 | チャプター付き動画 | .mp4 | - | 自習・確認用 |
| 最終 | チャプターリスト | .txt | chapters.txt変換 | YouTube概要欄用 |

### スコープ区分

| 区分 | 処理 | 備考 |
|------|------|------|
| Video Chapter Editor | yt-dlp, video-trim, video-chapters, yt-srt, whisper-remote | 青枠 |
| 外部処理 | whisperX, AI処理, LaTeX | オレンジ破線 |
