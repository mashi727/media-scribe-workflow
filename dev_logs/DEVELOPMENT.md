...
プロジェクトの根本思想と設計原則を明文化。

### 根本思想: 思考と作業の分離

クリエイティブな作業において、**思考（判断・決定）** と **作業（機械的処理）** は本質的に異なる。
エンコードの待ち時間が思考を中断する問題を解決するため、両者を時間的に分離する。

```
思考フェーズ（日中）              作業フェーズ（夜間）
├─ チャプター編集                ├─ Whisper文字起こし
├─ 除外区間の判断                ├─ ffmpegエンコード
├─ AI分析の方向性                ├─ AI分析実行
└─ 品質確認                      └─ PDF生成
        │                               │
        ▼                               ▼
   project.msw.yaml              朝起きたら完成
   （意思決定の凍結点）
```

### 設計原則

1. **配管と陶器（Plumbing & Porcelain）**
   - 配管: 単一機能CLI（yt-srt, vce-encode等）
   - 陶器: GUI + CLI Wrapper + Claude Codeコマンド

2. **GUIの責務限定**
   - 思考を支援する機能に特化（チャプター編集、プレビュー）
   - 作業は配管に委譲

3. **統一プロジェクトファイル**
   - `project.msw.yaml` が全ツール共通のインターフェース
   - 必要なセクションだけ使用（スケーラブル）

### 作成ドキュメント

`docs/architecture.md` を新規作成・拡充。
...
2. 設計思想: 配管と陶器
3. 配管ツール一覧
4. 陶器ツール一覧（GUI + CLI Wrapper）
5. スケーラブルなユースケース（レベル0〜4）
6. 責務分離マトリクス
7. 拡張パターン
8. 依存関係グラフ
9. 統一プロジェクトファイル仕様
...
- **設計判断の根拠が明確**: なぜGUIとCLIを分けるのか
- **新規開発者へのオンボーディング**: 思想から理解できる
- **拡張時の指針**: 新ツールをどの層に配置すべきか判断可能
